...
class Preprocessor:
    ...
    def preprocess_train_images(self, number, visualize = False):
        ...
        if number == -1:
            y = np.array([label if label in self.digits_of_interest else -1 for label in mnist_train_labels[filtered_indices]])
        else:
        ...
        return filtered_images.reshape(filtered_images.shape[0], -1), y

    def preprocess_test_images(self, number = -1, visualize = False):
